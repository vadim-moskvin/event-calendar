...
public class UserServiceTests : TestsBase
{
    ...
    public async Task Register_existing_login()
    {
        ...
        // Act + Assert
        await Assert.ThrowsAsync<BadRequestException>(() => UserService.Register("alice", "secret", Role.User));
    ...
    public async Task Login_unknown_user_or_wrong_password(string login, string password)
    {
        ...
        // Act + Assert
        await Assert.ThrowsAsync<NotAllowedException>(() => UserService.Login(login, password));
    }
}